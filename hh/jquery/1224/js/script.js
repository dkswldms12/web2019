$(document).ready(function(){
	...
	//클래스의 유무를 판단해서 클래스가 있으면 제거, 클래스가 없다면 추가
	$("#detail").click(function(){
		var $has_class=$("#detail").hasClass("active");//active가 있는지 없는지 찾는거 hasClass active가 존재하면 true
	...
	//hover이벤트 : 마우스가 영역 내부로 진입했을때 이벤트 발생 + 마우스가 영역 밖으로 빠져나왔을 때 이벤트 발생
	$(".card_img").hover(function(){//마우스가 영역내부로 진입시 이벤트 발생(mouseenter)
		$(".card_img").attr("src","../img/card.png");
	}, function(){//마우스가 영역 밖으로 빠져나왔을때 이벤트 발생(mouseleave)
		$(".card_img").attr("src","../img/ace.png");
	});


	//mousemove() : 마우스의 위치를 측정하는 이벤트, 활용범위는 마우스를 움직였을때 위치값을 받아와서 실행문을 작동
/*		$(document).mousemove(function(e){
		var x_position=e.pageX;//각 브라우저 기준의 마우스 위치의 x좌표
		var y_position=e.pageY;// 마우스 위치의 y좌표

		console.log("현재 마우스의 x좌표"+x_position);
		console.log("현재 마우스의 y좌표"+y_position);

	});

/*			$("#user_id").focus();//브라우저 유저가 모두 로딩되는 순간 해당하는 입력박스에 포커스를 강제 실해한다
...
*/
//window->scroll, resize, load, ready, mousemove
	...
	$(window).resize(function(){//값을 찍어주겠다
		resize_ev();
	});
